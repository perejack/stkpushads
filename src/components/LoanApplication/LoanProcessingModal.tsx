    ...
    const timer = setInterval(() => {
      setStep(prev => {
        ...
        setTimeout(() => {
          ...
          // Keep success screen visible longer so user sees ad
          setTimeout(onComplete, 5000);
        }, 1500);
        return prev;
      });
    }, 3000); // Increased from 2000 to 3000ms (3 seconds per step)
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-white rounded-2xl p-4 sm:p-8 max-w-md w-full mx-3 sm:mx-4 relative overflow-hidden max-h-[90vh] overflow-y-auto"
      >
        {!showSuccess ? (
          <>
            ...
            {/* Survey Ad - Make money while waiting! */}
            <div className="mb-4 sm:mb-6 bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl p-3 sm:p-6 border-2 border-purple-200">
              <div className="text-center mb-3 sm:mb-4">
                <div className="inline-block bg-gradient-to-r from-orange-500 to-red-500 text-white px-3 py-1.5 rounded-full text-[10px] sm:text-xs font-bold mb-2 animate-pulse">
                  💰 EARN WHILE YOU WAIT
                </div>
                ...
                <p className="text-xs sm:text-sm text-gray-600">
                  Why not make KES 300 right now?
                </p>
              </div>
              <AdContainer
                placement="application-waiting-survey"
                variant="card"
                maxAds={1}
